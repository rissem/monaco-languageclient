...
export class MonacoWorkspace implements Workspace {
  ...
  protected readonly onDidChangeTextDocumentEmitter =
    new Emitter<TextDocumentDidChangeEvent>();
  ...
  public applyEdit(workspaceEdit: WorkspaceEdit): Promise<boolean> {
    const edit: monaco.languages.WorkspaceEdit =
      this.p2m.asWorkspaceEdit(workspaceEdit);
    ...
    const models: { [uri: string]: monaco.editor.IModel } = edit.edits
      ? edit.edits.reduce(
          (acc: { [uri: string]: monaco.editor.IModel }, currentEdit) => {
            ...
            acc[textEdit.resource.toString()] = this._monaco.editor.getModel(
              textEdit.resource
    ...
    const editsByResource: {
      [uri: string]: monaco.editor.IIdentifiedSingleEditOperation[];
    } = edit.edits
      ? edit.edits.reduce(
          (
            acc: {
            ...
            acc[uri].push({
              range: this._monaco.Range.lift(
                textEdit.edit.range as monaco.IRange
              ),
